...
ctr -n k8s.io images pull --label io.cri-containerd.pinned=pinned --label io.cri-containerd.image=managed --platform ${cpu_architecture} --creds "AWS:$${ECR_PASSWORD}" "${img}"
%{ endfor ~}

# Public images
%{ for img in public_images ~}
ctr -n k8s.io images pull --label io.cri-containerd.pinned=pinned --label io.cri-containerd.image=managed --platform ${cpu_architecture} "${img}"
%{ endfor ~}
